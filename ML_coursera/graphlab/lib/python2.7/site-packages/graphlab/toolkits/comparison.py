...
def show_comparison(model_comp_sframe=None, models=None):
    r"""
    ...
    >>> graphlab.show_comparison([m1, m2], model_comp)
    """
    if type(model_comp_sframe) != _graphlab.SFrame:
        raise TypeError('"model_comp_sframe" must be a non empty SFrame')
    ...
    if len(models) != model_comp_sframe.num_rows():
        raise ValueError('Number of models must equal number of rows in "model_comp_sframe"')
    if not set(['metric', 'results','model']).issubset(set(model_comp_sframe.column_names())):
        raise ValueError('SFrame must be return value of graphlab.toolkits.compare_models()')

    model_dict = { row['model']: row['results'] for row in model_comp_sframe }

    ret = {}
    ret['dataset_label'] = model_comp_sframe['dataset'][0]
