...
from _util import deprecated as _deprecated
...
@_deprecated('"graphlab.deploy.copy_ec2_predictive_object" is deprecated, '
                'use "graphlab.deploy.copy_predictive_object" instead.')
def copy_ec2_predictive_object(source_ps, target_ps, source_po_name, target_po_name=None, update=False):
    ...
    '''
    copy_predictive_object(source_ps, target_ps, source_po_name, target_po_name=target_po_name, update=update)
...
def copy_predictive_object(source_ps, target_ps, source_po_name, target_po_name=None, update=False):
    ...
    if target_ps._schema_version - source_ps._schema_version > 1:
        raise RuntimeError('Only support copying from version %s or %s of '
            'predictive service. Your source predictive service has schema version %s' \
            % (target_ps._schema_version - 1, target_ps._schema_version, source_ps._schema_version))
