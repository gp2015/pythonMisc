...
v0: Save Predict Object along with dependent GraphLab objects in a zipped file.
    User can only add dependent GraphLab from a python object. (PO VERSION 1)
...
v4: Change Predicitve Service to use docker, as a result, the AMI image, the
    ...
    so it is an optional parameter in state.ini. (PO VERSION 3)
v5: Add model experimentation support, including adding endpoints and serialize
    endpoint policy (PO VERSION 3)
v6: Supported environment variables, has separate folder for dependent files
    and dependent packages. Predictive Object's dependent files are no longer picklered
    together wit the Predictive Object, and also support custom dependent packages,
    so the po.required_packages can also include path to a package file.
    (PO VERSION bumped to 6 directly from 3)
'''

PREDICTIVE_SERVICE_SCHEMA_VERSION = 6
PREDICTIVE_OBJECT_SCHEMA_VERSION = 6
LAST_SUPPORTED_PREDICTIVE_SERVICE_SCHEMA_VERSION = 3
LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES = 5
