...
CACHE_QUERY_TTL_SECS = "cache_query_ttl_secs"
CACHE_FEEDBACK_TTL_SECS = "cache_feedback_ttl_secs"

DISPLAY_NAME_MAPPING = {
    "Cache TTL on update (sec.)": CACHE_TTL_ON_UPDATE_SECS,
    "Cache max. memory (MB)": CACHE_MAX_MEMORY_MB,
    "Cache Query TTL (sec.)": CACHE_QUERY_TTL_SECS,
    "Cache Feedback TTL (sec.)": CACHE_FEEDBACK_TTL_SECS
}

def _get_supported_config(schema_version):
    '''
    Get all supported configurations given a schema version
    '''
    if schema_version >= 6:
        # cache_query_ttl_secs and cache_feedback_tl_secs are
        # added in schema version 6
        return [CACHE_TTL_ON_UPDATE_SECS, CACHE_MAX_MEMORY_MB,
                CACHE_QUERY_TTL_SECS, CACHE_FEEDBACK_TTL_SECS]
    else:
        return [CACHE_TTL_ON_UPDATE_SECS, CACHE_MAX_MEMORY_MB]
...
class SystemConfig(namedtuple("SystemConfig",
    ...
    def set_state_in_config(self, config, section_name, schema_version):
        param_name_mapping = _invert_display_name_mapping()
        supported_fields = _get_supported_config(schema_version)
