...
PREDICTIVE_SERVICE_NAME = 'Dato_Predictive_Service'
DATO_DISTRIBUTED_NAME = 'Dato_Distributed'
...
# Port 9000 - 9200 - new commander worker, used by Dato Distributed
# Port 19006 - used by predictive service (reddis).
DATO_DISTRIBUTED_PORT_MIN_NUM = 9000
DATO_DISTRIBUTED_PORT_MAX_NUM = 9200
REDIS_PORT = 9006
...
class _ProductType():
    DatoDistributed, PredictiveServices = range(2)
...
def _authorize_for_dato_distributed(security_group, cidr_ip):
    ...
    _authorize_for_security_group(security_group = security_group,
               ip_protocol = 'tcp',
               from_port = DATO_DISTRIBUTED_PORT_MIN_NUM,
               to_port = DATO_DISTRIBUTED_PORT_MAX_NUM,
               cidr_ip = cidr_ip)
...
def _get_security_group_config(product_type, region, credentials, security_group_name,
    ...
    elif product_type is _ProductType.DatoDistributed:
        security_group_default_name = DATO_DISTRIBUTED_NAME
    ...
    try:
        ...
        if security_group_name is not None and security_group_id is not None:
            ...
            if subnet_id is None:
                ...
                subnets = conn.get_all_subnets(filters={'vpc_id': security_group.vpc_id})
                if len(subnets) == 0:
                    raise Exception('Error: No Subnet inside VPC ' + str(security_group.vpc_id))
                subnet_id = subnets[0].id
                ...
                if(security_group_name == sg.name):
                    ...
                    if subnet_id is None:
                        ...
                        if len(subnets) == 0:
                            raise Exception('Error: No Subnet inside VPC ' + str(security_group.vpc_id))
                        subnet_id = subnets[0].id
                        # Note that if user gives a security_group_name and subnet_id,
                        # if that security_group_name exists in a different vpc from
                        # there is a mismatch.
                    break
            if security_group is None:
                if subnet_id is None:
...
def _setup_security_group(region, CIDR_rule, security_group_name,
    ...
    if product_type is _ProductType.PredictiveServices:
        ...
        _authorize_for_dato_distributed(security_group = security_group,
                                        cidr_ip = CIDR_rule)
    else:
        raise Exception("Product Type (Predictive Services or Dato Distributed is not specified.")
...
def _ec2_factory(instance_type, region=None, availability_zone=None,
    ...
    '''
    # Test that the unity_server binary works, for it is used to validate the
    # product_key
    from graphlab.connect.main import _verify_engine_binary
    _verify_engine_binary(config.server_bin)
