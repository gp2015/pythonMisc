...
def create(data, target=None, features=None, review_id=None,
    ...
    :py:class:`~graphlab._internal.search.SearchModel` that is created on the
    text data.
...
class ProductSentimentModel(_CustomModel):
    """
    ...
    _PRODUCT_SENTIMENT_MODEL_VERSION = 0
    ...
    def get_most_positive(self, keywords=None, groupby=None, k=10):
        ...
        return _get_most_extreme(self._state['reviews'],
                                 self._state['sentiment_scorer'],
                                 self._state['review_searcher'],
                                 self._state['review_id_column'],
                                 self._state['sentiment_score_column'],
                                 self._state['feature_column'],
                                 keywords=keywords,
    ...
    def sentiment_summary(self, keywords=None, groupby=None, k=10,
        ...
        return _sentsummary(self._state['reviews'],
                            self._state['sentiment_scorer'],
                            self._state['review_searcher'],
                            self._state['sentiment_score_column'],
                            self._state['feature_column'],
    ...
    def get(self, field):
        ...
        with the ``list_fields`` method. Two key fields include:

        +-----------------------+----------------------------------------------+
        |      Field            | Description                                  |
        +=======================+==============================================+
        | sentiment_scorer      | A trained SentimentAnalysisModel that scores |
        |                       | review data in terms of positive or negative |
        |                       | sentiment.                                   |
        +-----------------------+----------------------------------------------+
        | review_searcher       | A SearchModel object that can query the text |
        |                       | data provided for chosen search terms.       |
        +-----------------------+----------------------------------------------+
    ...
    def get_current_options(self):
        ...
        current model.
        """
        opts = {'target'   : self._state['target'],
