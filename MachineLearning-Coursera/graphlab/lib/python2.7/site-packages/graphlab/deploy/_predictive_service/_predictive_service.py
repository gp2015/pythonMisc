...
from graphlab.util.file_util import is_s3_path as _is_s3_path, \
    is_path as _is_path, retry as _retry, expand_full_path as _expand_full_path, \
    mkdir as _mkdir, exists as _exists
...
from . import PREDICTIVE_SERVICE_SCHEMA_VERSION, INTERNAL_PREDICTIVE_OBJECT_NAMES, \
    ...
    LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES
...
from _environment_variables import EnvironmentVariables as _EnvironmentVariables
...
_VALID_CONFIG_PARAMS = {
    ...
    "cache_ttl_on_update_secs": (lambda v: True if isinstance(v, int) and v >= 0 else False),
     "cache_query_ttl_secs": (lambda v: True if isinstance(v, int) and v >= 0 else False),
    "cache_feedback_ttl_secs": (lambda v: True if isinstance(v, int) and v >= 0 else False)
}
...
class PredictiveService(_Artifact):
    ...
    def __str__(self):
        ...
        ret += "\nDeployed endpoints:\n"
        for (name, po_info) in sorted(self._endpoints.iteritems()):
                ret += '\t%s [%s]\n' % \
                    (name, po_info.get('type', 'model'))

        if self.environment_variables._original_value:
            ret += '\nDeployed environment variables:\n'
            for k, v in sorted(self.environment_variables._original_value.iteritems()):
                ret += '\t%s\n' % k
        ...
        if self.environment_variables._dirty:
            ret += '\nThere are pending environment variable changes, use print ps.environment_variables to see them.'
    ...
    def endpoints(self):
        ...
        for (endpoint_name, endpoint_info) in self._endpoints.iteritems():
            ...
            if 'endpoint_obj' in return_endpoint_info:
    ...
    def environment_variables(self):
        '''
        Returns the environment variables that have been set in the predictive service.
        '''
    ...
    @property
    def pending_endpoint_changes(self):
        '''
        Return all currently pending endpoint changes.
    ...
    def api_key(self):
        '''
        Returns the API key for this Predictive Service.
    ...
    def set_api_key(self, api_key):
        '''
        Set or change the API key for this Predictive Service

        Parameters
        ----------
        api_key : str
            The API key used to query the Predictive Service.

        Examples
        --------

            >>> deployment.set_api_key('e67e65d1-411d-4589-bf0d-c4041c0efc86')

        '''
        self._ensure_not_terminated()
        self._ensure_no_local_changes()

        if not api_key or not isinstance(api_key, basestring):
            raise ValueError('"api_key" value must be a string.')

        # set the current api_key
        ...
        try:
            ...
            _logger.warn("Unable to change the Predictive Service API key: %s" %
                         (e.message))
            self._api_key = old_api_key
            return

        try:
            self._environment.poke(self.admin_key)
            self._environment.clear_connection()
            _logger.info("Predictive Service API key has been changed, please wait a "\
                "while for all nodes to take effect.")
        except _ConnectionError as e:
            _logger.warn("Unable to contact all nodes. It is possible that not all nodes "\
                         "are up to date with the API key. Please use get_status('node') "\
                         "to check and recover node issues. Error: %s" %
                         (e.message))
    ...
    def admin_key(self):
        ...
        '''
        self._ensure_not_terminated()
        return self._environment.admin_key

    def set_admin_key(self, admin_key):
        '''
        Set or change the admin key for this predictive service

        parameters
        ----------
        admin_key : str
            the admin key used to manage the predictive service.

        examples
        --------

            >>> deployment.set_admin_key('8f46993e-9784-4b34-a317-12a9dd56e5d2')

        '''
        self._ensure_not_terminated()
        self._ensure_no_local_changes()

        if not admin_key or not isinstance(admin_key, basestring):
            raise ValueError('"admin_key" value must be a string.')

        # set the current admin_key
        old_admin_key = self._environment.admin_key
        self._environment.admin_key = admin_key

        try:
            self._save_state()
        except _ConnectionError as e:
            _logger.warn("Unable to change the Predictive Service admin key: %s" %
                         (e.message))
            self._environment.admin_key = old_admin_key
            return

        try:
            self._environment.poke(old_admin_key)
            self._environment.clear_connection()
            _logger.info("Predictive Service admin key has been changed, please wait a "\
                "while for all nodes to take effect.")
        except _ConnectionError as e:
            _logger.warn("Unable to contact all nodes. It is possible that not all nodes "\
                         "are up to date with the admin key. Please use get_status('node') "\
                         "to check and recover node issues. Error: %s" %
                         (e.message))
    ...
    def __init__(self, name, state_path, description, api_key, admin_key,
        ...
        self._local_changes = {}
        self._endpoints = {}
        self._environment_variables = _EnvironmentVariables()
    ...
    def _get_nodes_status(self):
        ...
        host_status = []
        if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
            result = self._environment.get_status_from_nodes_directly()
        else:
            result = self._environment.get_status()
        for host in result:
    ...
    def get_status(self, view = None):
        ...
        if not view:
            if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
                return self._environment.get_status_from_nodes_directly()
            else:
    ...
    def query(self, name, **kwargs):
        ...
        try:
            ...
            return self._environment.query(name, self._api_key, timeout=timeout, **kwargs)
    ...
    def feedback(self, request_id, **kwargs):
        ...
        try:
            timeout = self.query_timeout if hasattr(self, 'query_timeout') else 10
            result = self._environment.feedback(request_id, self._api_key,
    ...
    def update(self, name, obj, description = ''):
        ...
        if not self._endpoints.has_key(name):
            if not self.pending_endpoint_changes.has_key(name):
    ...
    def remove(self, name):
        ...
        if self._endpoints.has_key(name):
            ...
            _logger.info("Pending endpoint '%s' is removed." % name)
            found = True
    ...
    def apply_changes(self):
        ...
        See Also
        --------
        add, update, remove, alias, pending_endpoint_changes
        '''
        self._ensure_not_terminated()

        if not self._has_pending_changes():
            _logger.info("There are pending_endpoint_changes. No action is taken.")
            return

        # persist endpoints to service state
        if len(self._local_changes) > 0:
            _logger.info('Persisting endpoint changes.')
            self._persist_pending_changes()

        if self._environment_variables._dirty:
            _logger.info('Persisting environment variable changes.')
        ...
        try:
            if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
                self._environment.poke_from_nodes_directly(self.admin_key)
            else:
                self._environment.poke(self.admin_key)
            self._environment.clear_connection()
        except _ConnectionError as e:
    ...
    def clear_pending_changes(self):
        ...
        If there are environment_variables changes, then that change is cleared too.
        '''
        self._ensure_not_terminated()

        _logger.info('Clearing all pending changes.')
        self._local_changes = {}
        self._environment_variables._reset()
    ...
    def _recover_cache(self, max_retries=3):
        ...
        def toggle(is_enabled, name):
            if is_enabled:
                if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
                    self._environment.cache_enable_from_nodes_directly(name)
                else:
                    self._environment.cache_enable(name)
            else:
                if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
                    self._environment.cache_disable_from_nodes_directly(name)
                else:
        ...
        # disable the cache
        if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
            self._environment.cache_disable_from_nodes_directly(None)
            self._environment.cache_enable_from_nodes_directly(None, True)
        else:
    ...
    def _upload_file(self, local_path, remote_path = ''):
        '''
        Add files to Prdictive Service, this could be any file type. The files are
        ...
        ---------
            ...
            # upload file my_code.py to Predictive Service node under
            # <DEPENDENT-FILES-ROOT>/my_code.py
            >>> ps._upload_file('path/to/my/code/my_code.py', 'my_code.py')

            # This copies file my_code.py to Predictive Service node under
            # <DEPENDENT-FILES-ROOT>/my_code_folder folder
            >>> ps._upload_file('/code/python/my_code.py', 'my_code_folder')

            # This copies all files under a directory called my_code_folder to remote
            # Predictive Service node under <DEPENDENT-FILES-ROOT>
            >>> ps._upload_file('/code/python/my_code_folder/*')

            # This copies all files under a directory called my_code_folder to remote
            # Predictive Service node under <DEPENDENT-FILES-ROOT>/my_code_folder
            >>> ps._upload_file('/code/python/my_code_folder')

            # This copies all files under a directory called my_code_folder to remote
            # Predictive Service node under <DEPENDENT-FILES-ROOT>/remote_folder/my_code_folder
            >>> ps._upload_file('/code/python/my_code_folder', 'remote_folder')

        Notes
        -----
        If there are any Predictive Object or endpoint policy dependent on these
        newly uploaded files, these object needs to be updated in order to
        use the newly loaded files.

        If your custom code needs the files/modules defined, you may assume the
        files are available to you in sys.path. In your code, you may simply
        import your module like:

            >>> ps._upload_file('my_code_folder')

            def my_func(my_input):
                from my_code_folder import my_func
                ...
        '''
        if not isinstance(remote_path, basestring):
            raise TypeError('Remote path has to be a string')
        ...
        # Remove trailing '/' or '\'
        if _os.path.split(local_path)[1] == '':
            local_path = _os.path.split(local_path)[0]

        # # Make sure the dependent file path exists
        dependent_file_path = PredictiveService._get_dependent_file_path(self._state_path)
        if not _exists(dependent_file_path, aws_credentials = self.aws_credentials):
            _mkdir(dependent_file_path)

        files = _glob.glob(local_path)
        if len(files) == 0:
            raise ValueError('Cannot find any files/directories to upload.')

        for f in files:
            file_or_dir_name = _os.path.split(f)[1]
            if remote_path == '' or remote_path is None:
                remote_file_path = dependent_file_path + '/' + file_or_dir_name
            else:
                remote_file_path = dependent_file_path + '/' + remote_path
            self._environment.upload_file(f, remote_file_path, aws_credentials = self.aws_credentials)
    ...
    def _remove_file(self, remote_path):
        '''
        Remove specific files or a whole folder from Predictive Service

        Warning -- if the files are used by your models, this action may cause
        your working models not working anymore. Be careful of removing files.

        Parameters
        -----------

        remote_path : str
            Remote file or folder to be removed. The path must be a relative
            path. If remote_path is a folder, then all files under the folder,
            including the folder is removed.

        Examples
        ---------

            To remove a file
            >>> ps._remove_file('path/to/my/file.txt')

            To remove a folder recursively
            >>> ps._remove_file('path/to/folder/')
        ...
        Notes
        ------
        When Predictive Service uses S3 to store state, removing files is
        equivalent to removing all keys with the given prefix. So be careful
        to put '/' at the end of the path if you only want to remove the files
        under certain 'path'.

        '''
        if remote_path == '' or remote_path is None:
            raise ValueError('Remote file path can not be empty. If you want to '
                            'remove all files, use "*" instead.')
        ...
        dependent_files_path = PredictiveService._get_dependent_file_path(self._state_path)

        # special character: '*', remove all dependent files
        if remote_path == '*':
            remote_path = dependent_files_path
        else:
            remote_path = dependent_files_path + '/' + remote_path

        self._environment.remove_file(remote_path, self.aws_credentials)
    ...
    def _list_file(self, remote_path = ''):
        '''
        List all files in predictive Service under the given folder

        remote_path : str, optional
            Remote folder to list files from.
            This must be a relative path.
            Only one layer of dependendent files are listed.

        Examples
        ---------

            To list all files/folders in root
            >>> ps.l_ist_file()

            To list all files under folder "my_folder"
            >>> ps._list_file('my_folder')

            To list a deeper folder:
            >>> ps._list_file('path/to/where/you/want')
        '''
        ...
        return self._environment.list_file(
            PredictiveService._get_dependent_file_path(self._state_path) + '/' + remote_path,
    ...
    def list_custom_packages(self):
        ...
        '''
        return self._environment.list_file(
            PredictiveService._get_dependent_package_path(self._state_path),
            self.aws_credentials)
    ...
    def _get_latest_state(self):
        ...
        self._system_config = _SystemConfig.from_config_parser(remote_cfg,
                                        PredictiveService._SYSTEM_SECTION_NAME)

        if remote_cfg.has_section(PredictiveService._ENVIRONMENT_VARIABLE_SECTION_NAME):
            self._environment_variables = _EnvironmentVariables._read_from_config_file(
                            remote_cfg, PredictiveService._ENVIRONMENT_VARIABLE_SECTION_NAME)
        ...
        # update the api key
        if remote_cfg.has_option(PredictiveService._SERVICE_INFO_SECTION_NAME, 'API Key'):
            self._api_key = remote_cfg.get(PredictiveService._SERVICE_INFO_SECTION_NAME, 'API Key')
    ...
    def get_result_logs(self, start_time = None, end_time = None):
        """
    ...
    def show(self):
        ...
        if isinstance(_canvas.get_target(), _canvas.target.IPythonTarget):
            _logger.warn("Cannot show Predictive Service in IPython Notebook. "
                         "Please use interactive target to show Predictive Service: "
    ...
    def cache_enable(self, name=None, restart=True, max_retries=30):
        """
    ...
    def cache_clear(self, name=None):
        ...
        self._disallow_cache_operation_for_non_model(name)
        if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
            self._environment.cache_clear_from_nodes_directly(name)
        else:
    ...
    def flush_logs(self):
        ...
        self._ensure_not_terminated()
        if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
            self._environment.flush_logs_from_nodes_directly()
        else:
    ...
    def reconfigure(self, params):
        ...
        Update the TTL for querying the cache:

            >>> ps.reconfigure({"cache_query_ttl_secs": 120})

        Update the TTL for giving feedback on the query:

            >>> ps.reconfigure({"cache_feedback_ttl_secs": 120})
        ...
        try:
            self._save_state()
            if self._schema_version <= LAST_SUPPORTED_PREDICTIVE_SERVICE_COMMUNICATES_DIRECTLY_WITH_NODES:
                self._environment.reconfigure_from_nodes_directly(self._system_config)
            else:
        ...
        if len(self._local_changes) > 0:
            for (endpoint_name, info) in self._local_changes.iteritems():
                if info['action'] != 'remove' and \
                    ...
                    if hasattr(info['endpoint_obj'], 'required_files') and \
                                            info['endpoint_obj'].required_files:
                        for relativepath, realpath in info['endpoint_obj'].required_files.iteritems():
                            self._upload_file(realpath, relativepath)

                    # upload dependent package files for Predictive Object
                    remote_base_path = PredictiveService._get_dependent_package_path(self._state_path)
                    if hasattr(info['endpoint_obj'], 'required_packages') and \
                                            info['endpoint_obj'].required_packages:
                        for package_name_or_file in info['endpoint_obj'].required_packages:
                            local_file = _expand_full_path(package_name_or_file)
                            if _os.path.isfile(local_file):
                                package_name = _os.path.basename(local_file)
                                self._environment.upload_file(
                                    local_path = local_file,
                                    remote_path = _os.path.join(remote_base_path, package_name),
    ...
    def _save_state(self):
        ...
        if self._has_remote_state_changed():
            raise IOError("Can not save changes. The Predictive Service has changed. Please "
                          "reload your Predictive Service first using predictive_service.load.")
        ...
        self._system_config.set_state_in_config(
            state, PredictiveService._SYSTEM_SECTION_NAME, self._schema_version)

        # Save environment variables
        if self._environment_variables and self._schema_version >= 6:
            state.add_section(PredictiveService._ENVIRONMENT_VARIABLE_SECTION_NAME)
            self._environment_variables._write_to_config_file(
                state, PredictiveService._ENVIRONMENT_VARIABLE_SECTION_NAME)
    ...
    def _has_pending_changes(self):
        return len(self._local_changes) > 0 or self.environment_variables._dirty

    @staticmethod
    def _get_dependent_file_path(state_path):
        return _os.path.join(state_path, 'dependent_files')

    @staticmethod
    def _get_dependent_package_path(state_path):
        return _os.path.join(state_path, 'dependent_packages')
