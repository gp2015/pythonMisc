...
from graphlab.util.file_util import parse_s3_path, s3_delete_key, exists, \
                            s3_recursive_delete, retry, mkdir, copy_from_local
...
def predictive_service_environment_factory(env_info):
    ...
    if env_info.has_key('certificate_is_self_signed') and \
...
class PredictiveServiceEnvironment:
    ...
    def _poke_host(self, host_name, admin_key):
        ...
        data = json.dumps({'admin_key': admin_key})
    ...
    @retry()
    def cache_enable_from_nodes_directly(self, name, restart=True):
    ...
    @retry()
    def cache_disable_from_nodes_directly(self, name):
        for host in self._host_addrs():
            try:
    ...
    @retry()
    def cache_clear_from_nodes_directly(self, name):
        for host in self._host_addrs():
            try:
    ...
    @retry()
    def flush_logs_from_nodes_directly(self):
        for host in self._host_addrs():
            try:
    ...
    @retry()
    def reconfigure_from_nodes_directly(self, system_conf):
        for host in self._host_addrs():
            try:
...
class LocalPredictiveServiceEnvironment(PredictiveServiceEnvironment):
    ...
    #This is kept for backwards compatibility for Predictive Services
    #with version <= 5.
    def get_status_from_nodes_directly(self):
        dns_name, port = self.load_balancer_dns_name.split(':')
        ...
        return [{
            ...
            'service_version': node_status.get('service_version') if not node_status.has_key('error') else None
        }]
    ...
    def poke_from_nodes_directly(self, admin_key):
        self._poke_host('localhost', admin_key)
...
class DockerPredictiveServiceEnvironment(PredictiveServiceEnvironment):
    ...
    #This is kept for backwards compatibility for Predictive Services
    #with version < 5.
    def get_status_from_nodes_directly(self, _show_errors=True):
        result = []
    ...
    def _get_metrics(self, name, ps_name, is_po, start_time, end_time, period):
        ...
        # get metrics
        if not name:
            # when name is None, return requests, latency and exceptions
    ...
    def poke_from_nodes_directly(self, admin_key):
        for hostname in self._host_addrs():
            try:
                self._poke_host(hostname, admin_key=None)
            except Exception as e:
...
class Ec2PredictiveServiceEnvironment(PredictiveServiceEnvironment):
    ...
    def _get_metrics(self, name, ps_name, is_po, start_time, end_time, period=300):
        ...
        if not name:
            ...
            exceptions_info = self._get_metric_info('exceptions')
            result['exceptions'] = cw.get_metric_statistics(period, start_time, end_time,
                            'exceptions', ps_name, [latency_info['statistics']], {}, exceptions_info['unit'])
        elif name and not is_po:
    ...
    def get_status_from_nodes_directly(self, _show_errors=True):
        ...
        for host in load_balancer.get_instance_health():
            ...
            if _show_errors and node_status.has_key('error'):
                ...
                raise RuntimeError("Cannot get node status")
    ...
    def poke_from_nodes_directly(self, admin_key):
        for instance in self._get_all_hosts():
            try:
                self._poke_host(instance.public_dns_name, admin_key)
            except Exception as e:
...
class TestEc2Env(Ec2PredictiveServiceEnvironment):
    ...
    def __init__(self, aws_credentials):
        Ec2PredictiveServiceEnvironment.__init__(self,
                ...
                'admin_key', None, None, aws_credentials, PORT_DEFAULT_NUM)
    ...
    def _get_metrics(self, name, ps_name, is_po, start_time, end_time, period):
        ...
        while start < end_time:
            ...
            exceptions.append({'sum': 1.0, 'time': start, 'unit':'Count'})
        ...
        result = {}
        if not name:
            result = {'requests': num_requests, 'latency': latency, 'exceptions': exceptions}
            ...
            result = {'test_po::requests': num_requests,
                      'test_po::latency': latency,
                      'test_po::exceptions': exceptions}
