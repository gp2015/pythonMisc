    ...
    **Add/Remove TimeSeries Columns**
    ...
    def resample(self, period, downsample_method='avg', upsample_method=None,
        ...
        downsample_method : agg | dict[col:agg]
            Method for down- or re-sampling. Basically it determines the aggregate
            method for non-empty buckets. Default to AVG for downsampling.
            where col : str, represents the column names, and `agg` is one of

            * graphlab.timeseries.downsample.AVG
        ...
        upsample_method : agg
            Filling method for upsampling. It determines how to fill empty
            buckets, where `agg` is either

            * graphlab.timeseries.upsample.NEAREST
        ...
        First we resample the data with 'sum' as ``downsample method`` and
        missing values are filled with their 'nearest' index value.
        ...
        >>> t_resample = ts.resample(dt.timedelta(days=3), 
        ...                          downsample_method='avg',
        ...                          upsample_method=None, label='left')
        >>> print t_resample
        +---------------------+------+
        |          b          |  a   |
        +---------------------+------+
        ...
        Now we change bucket labeling to 'right'. So buckets are labeled with
        their right-edge index.
        ...
        We can also specify which side of the bucket is closed (default is
        'left').

        >>> t_resample = ts.resample(dt.timedelta(days=3), 
        ...                          downsample_method='avg',
        ...                          upsample_method=None, closed='right',
        ...                          label='right')
        >>> print t_resample
        +---------------------+------+
        |          b          |  a   |
    ...
    def __getitem__(self, key):
        ...
        # to TimeSeries. In case ``gi_ret`` is an SFrame, we automatically add the 
        # TimeSeries index column to ``gi_ret`` (if it is not already threre) and convert 
        # it to a TimeSeries object. The idea is that TimeSeries.__getitem__() should not return
        ...
        if gi_ret.__class__.__name__ == 'SFrame':
            ...
            if not self.index_col_name in cnames:
                gi_ret.add_column(self[self.index_col_name],name=self.index_col_name)
            try:
                return TimeSeries(gi_ret, index=self.index_col_name,is_sorted=True) 
            except:
                pass
        return gi_ret
